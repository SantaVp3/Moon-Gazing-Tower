...
type ScanPipeline struct {
	...
	subfinderScanner   *scanner.SubfinderScanner
	httpxScanner       *scanner.HttpxScanner
...
func NewScanPipeline(task *models.Task, taskService *TaskService, resultService *ResultService) *ScanPipeline {
	...
	return &ScanPipeline{
		...
		taskService:        taskService,
		resultService:      resultService,
		domainScanner:      scanner.NewDomainScanner(10),
		subfinderScanner:   scanner.NewSubfinderScanner(),
		httpxScanner:       scanner.NewHttpxScanner(10),
		rustScanScanner:    scanner.NewRustScanScanner(),
		katanaScanner:      scanner.NewKatanaScanner(),
		radScanner:         scanner.NewRadScanner(),
		fingerprintScanner: scanner.NewFingerprintScanner(10),
		contentScanner:     scanner.NewContentScanner(10),
		vulnScanner:        scanner.NewVulnScanner(10),
		takeoverScanner:    scanner.NewTakeoverScanner(10),
