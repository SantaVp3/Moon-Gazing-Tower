...
type SubdomainScanModule struct {
	BaseModule
	subfinder    *scanner.SubfinderScanner
...
func NewSubdomainScanModule(ctx context.Context, nextModule ModuleRunner, maxEnumTime int, resolveIP bool) *SubdomainScanModule {
	...
	m := &SubdomainScanModule{
		...
		subfinder:   scanner.NewSubfinderScanner(),
...
// 执行子域名接管检测、DNS解析等
type SubdomainSecurityModule struct {
	BaseModule
...
// NewSubdomainSecurityModule 创建子域名安全检测模块
func NewSubdomainSecurityModule(ctx context.Context, nextModule ModuleRunner, checkTakeover bool) *SubdomainSecurityModule {
	m := &SubdomainSecurityModule{
		BaseModule: BaseModule{
			name:       "SubdomainSecurity",
			ctx:        ctx,
			nextModule: nextModule,
			dupChecker: NewDuplicateChecker(),
		},
		domainScanner: scanner.NewDomainScanner(10), // 默认10并发
...
// ModuleRun 运行模块
func (m *SubdomainSecurityModule) ModuleRun() error {
	var allWg sync.WaitGroup
	var resultWg sync.WaitGroup
	var nextModuleRun sync.WaitGroup
...
func (m *SubdomainSecurityModule) checkSubdomain(sr SubdomainResult) {
	...
	rootDomain := scanner.ExtractRootDomain(sr.Domain)
