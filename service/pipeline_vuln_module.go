...
func NewVulnScanModule(ctx context.Context, nextModule ModuleRunner, concurrency int) *VulnScanModule {
	...
	m := &VulnScanModule{
		...
		vulnScanner: scanner.NewVulnScanner(concurrency),
...
func (m *VulnScanModule) scanVulnerabilities(target string) {
	...
	if len(m.tags) > 0 {
		var tagFiltered []*scanner.POCTemplate
...
func intersectTemplates(a, b []*scanner.POCTemplate) []*scanner.POCTemplate {
	set := make(map[string]bool)
	for _, t := range a {
		set[t.ID] = true
	}

	var result []*scanner.POCTemplate
	for _, t := range b {
		if set[t.ID] {
